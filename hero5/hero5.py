...
def lr_term1():
   subprocess.Popen(["xfce4-terminal", "--working-directory=./hero5/56.101/"])

def lr_term2():
   subprocess.Popen(["xfce4-terminal", "--working-directory=./hero5/56.102/"])
...
def tcpreplay():
   if button["text"] == "Start Hero #1":
      ...
      subprocess.Popen(["tcpreplay", "--intf1=eth0", "./hero5/network.pcap"])
      return
   else:
...
B = Tkinter.Button(top, text ="Intro", command = hero5intro)
