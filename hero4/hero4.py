...
def lr_term1():
   subprocess.Popen(["xfce4-terminal", "--working-directory=./hero4/56.101/"])

def lr_term2():
   subprocess.Popen(["xfce4-terminal", "--working-directory=./hero4/56.102/"])
...
def tcpreplay():
   if button["text"] == "Start Hero #1":
      ...
      subprocess.Popen(["tcpreplay", "--intf1=eth0", "./hero4/network.pcap"])
      return
   else:
...
B = Tkinter.Button(top, text ="Intro", command = hero4intro)
